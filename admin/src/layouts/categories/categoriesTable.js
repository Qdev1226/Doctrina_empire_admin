...
const CategoriesTable = ({ open, setOpen, currentId, setCurrentId, searchKey }) => {
  ...
  const handleEdit = (selectedCategoryId) => {
    setOpen((open) => !open)
    setCurrentId(selectedCategoryId)
  }
  ...
  }, [searchKey, categories.refresh, page])
  ...
  if (categoriesData.length === 0) {
    return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', paddingTop: '20px', margin: '80px' }}>
      <NoData />
      <p style={{ padding: '40px', color: 'gray', textAlign: 'center' }}>No Categorys yet. Click the ADD NEW BUTTON to add category</p>
    </div>
  }

  const rows = categoriesData
  const emptyRows = rowsPerPage - categoriesData.length
  ...
  const cellStyle = { fontSize: 14, borderBottom: 'none', padding: '8px 18px', textAlign: 'left', color: '#7B809A' }
    ...
    <Table aria-label="custom pagination table">
      <TableHead style={{ display: 'table-header-group' }}>
        <TableRow style={{ borderBottom: '1px solid #ddd' }}>
          ...
          <TableCell style={headerStyle}>CATEGORY NAME</TableCell>
        ...
        {rows.length > 0 &&
          rows.map((row, index) => (
            <TableRow key={row._id}>
              <TableCell style={{ ...cellStyle, width: '10%', textAlign: 'center' }}>{page * rowsPerPage + index + 1}</TableCell>
              <TableCell style={{ ...cellStyle, width: "60%" }}>{row.name}</TableCell>
